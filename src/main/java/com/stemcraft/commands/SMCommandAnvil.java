...
public class SMCommandAnvil extends SMCommand {

        public SMCommandAnvil() {
            super("anvil");
            permission("stemcraft.command.anvil");
            description("Opens the anvil interface");
        ...
        public void execute(SMCommandContext ctx) {
            ...
            if(player.value != ctx.player && !ctx.hasPermission("stemcraft.command.anvil.others")) {
                ctx.error("You do not have permission to open the anvil interface for other players");
                return;
            }

            SMUtilsPlayer.openAnvil(player.value, null, true);
        }
}
